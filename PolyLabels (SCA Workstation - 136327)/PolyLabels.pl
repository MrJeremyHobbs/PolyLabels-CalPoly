...
my $auto_print          = $cfg->param("printer.auto_print");
my $apiKey              = $cfg->param("misc.apiKey");
my $ver                 = $cfg->param("misc.ver");
my $SCA_Printing        = $cfg->param("misc.SCA_Printing");
...
my $ent = $top_frame->Entry(-font => 'Consolas 16')->pack(-side => 'top', -fill => 'both', -expand => 1);
   ...
   $ent->focus;
my $printer_img = $mw->Photo(-format => 'png', -file=>'printer.png');
...
sub generateFinalLabel
{
	...
	if ($library eq 'CPSLO_SCA' & $SCA_Printing eq 'active') {
        print "heeeeyyy";
...
sub changeSettings
{
    ...
    my $f2 = $om->Frame()->pack(-fill=>'both');   
       ...
       $f2->Optionmenu(-variable => \$auto_print, -options => ["yes", "no"], -command => [sub {$cfg->param("printer.auto_print", $auto_print)}])->pack(-side=>'right');
