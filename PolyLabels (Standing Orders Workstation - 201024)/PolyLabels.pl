...
my $mw = MainWindow->new;
   ...
   $mw->toplevel->iconbitmap("logo_small.ico");

#/Canvas and logo
my $canvas = $mw->Canvas(-width => 0, -height => 110)->pack(-side => 'top', -expand => 1, -fill => 'both');
my $logo   = $canvas->Photo(-format => 'png', -file => "logo_large.png");
   $canvas->createImage(165,60, -image => $logo);

#/Entry and button frame
my $top_frame = $mw->Frame()->pack( -side => 'top', -fill => 'both');
my $ent = $top_frame->Entry(-font => 'Consolas 12')->pack(-side => 'top', -fill => 'both', -expand => 1);
   $ent->bind('<Key-Return>', \&main); #/Sends barcode to main sub on pressing enter
   $ent->focus;
my $but = $top_frame->Button(-text => "Scan", -font => "Arial 11", -command =>\&main)->pack(-side => 'top', -fill => 'both', -expand => 1); #/Sends barcode to main sub on button press
...
sub main
{
    ...
    #/Get Alma XML
    my $alma_xml = getAlmaXML("https://api-na.hosted.exlibrisgroup.com/almaws/v1/items?view=label&item_barcode=$barcode&apikey=$apiKey");
    my $xml_OK   = checkXML($alma_xml);
    return if $xml_OK eq 'false';   
    my $xml_hash = createXMLHash($alma_xml);
    
    #/Get Holdings Data
    my $mms_id = $xml_hash->{bib_data}->{mms_id};
    my $holding_id = $xml_hash->{holding_data}->{holding_id};
    my $holdings_xml = getAlmaXML("https://api-eu.hosted.exlibrisgroup.com/almaws/v1/bibs/$mms_id/holdings/$holding_id?apikey=$apiKey");
    my $parsed_holdings = parseHoldings($holdings_xml);
    $xml_hash->{parsed_holdings} = $parsed_holdings;
    ...
    insertStatus("Current Location: $location");
    ...
    my $label = generateFinalLabel($location);
...
sub parseHoldings
{
    ...
    my $special_notes = $1 if $holdings_xml =~ /(CHECK NOTE.*)/;
    my @notes = split / \| /, $special_notes;
    my $parsed_holdings = '-' x $pocket_label_width;
    foreach my $n (@notes) {
        if ($n =~ /CHECK NOTE/) {
            $parsed_holdings = $parsed_holdings . "$n\n";
        }
    }
    $parsed_holdings =~ s/CHECK NOTE:/***/g;
    chomp $parsed_holdings;
...
sub printCustomLabel
{
    ...
    my $location = $1 if  $status_txt =~ /Current Location: (.*)/;
    
    my ($spine, $pocket) = getLabelText();
	my $label_OK = checkSpineLabel($spine);
    my $label = generateFinalLabel($location) if $label_OK eq 'true';
	printLabel($label, $location);
